...
class PhdUltraPump(Pump):
    ...
    def __init__(self, port: str = "COM19", baudrate: int = 9600,
        ...
        self.pause = pause
        # open serial port (may raise serial.SerialException)
...
class PhdUltraPump(Pump):
    ...
    def __init__(self, port="COM19", baudrate=9600,
                 timeout=1.0, address=0, pause=0.10):
        self.address, self.pause = address, pause
        self.ser = serial.Serial(port, baudrate,
                                 parity=serial.PARITY_NONE,
                                 stopbits=serial.STOPBITS_ONE,
                                 bytesize=serial.EIGHTBITS,
    ...
    def select_syringe(self, volume_ml: float) -> str:
        """
        Configure the pump for one of the predefined Air-Tite syringes.

        Parameters
        ----------
        volume_ml : float
            Nominal syringe volume in mL (must match a key in AIR_TITE_SYRINGES).

        Returns
        -------
        Pump reply string from the diameter command.
        """
        diameter = self.AIR_TITE_SYRINGES.get(volume_ml)
        if diameter is None:
            raise ValueError(f"Unsupported syringe size {volume_ml} mL. "
                             f"Choose from {list(self.AIR_TITE_SYRINGES.keys())}")
        self.syringe_size_ml = volume_ml
        self.set_syringe_volume(volume_ml * 1000, "ul")   # informational
        return self.set_diameter(diameter)

    def set_diameter(self, mm: float) -> str:
        return self.send(f"diameter {mm:.3f}")

    def set_syringe_volume(self, vol_ul: float, unit="ul") -> str:
        return self.send(f"svolume {vol_ul} {unit}")

    # ---------- run control ----------
    def _set_rate_sync(self, rate_ul_min: float, unit="ul/min") -> str:
        return self.send(f"irate {rate_ul_min:.4g} {unit}")

    def clear_volume_counter(self) -> str:
        return self.send("cvolume")

    def set_target_volume(self, vol_ul: float, unit="ul") -> str:
        return self.send(f"tvolume {vol_ul} {unit}")
    ...
    def _dispense_sync(self, vol_ul: float, rate_ul_min: float) -> None:
        """
        Infuse *vol_ul* at *rate_ul_min* and let the pump stop automatically.

        Non-blocking: returns immediately after starting the run.
        """
        if rate_ul_min <= 0:
            raise ValueError("rate_ul_min must be > 0")
        if not getattr(self, "ser", None):
        ...
        self.set_target_volume(vol_ul, "ul")
        self.run()                         # pump stops when target reached
        duration_sec = vol_ul / rate_ul_min * 60  # (µL) ÷ (µL · min⁻¹) × 60 s / min
        duration_sec = duration_sec + 2  # An extra 2 second padding was added here to ensure that it's completed
        time.sleep(duration_sec)
    ...
    # ---------------- Async wrappers to satisfy Pump ABC ----------------
    async def dispense(self, vol_ul: float, rate_ul_min: float) -> None:  # type: ignore[override]
        """Async wrapper for dispense: runs blocking dispense in a thread."""
        return await asyncio.to_thread(self._dispense_sync, vol_ul, rate_ul_min)
